...
public class Bootstrap extends Job{
	...
	public void doJob() throws IllegalAccessException{
		...
		if(database.isEmpty()){
			...
			calJackWork.addEvent("Event1", calJackWork, 
					start.getTime().toString(), 
					end.getTime().toString(), true);
			
			start.set(2011, 10, 22, 12, 0);
			start.set(2011, 10, 22, 20, 0);
			calJackWork.addEvent("Event2", calJackWork, 
					start.getTime().toString(), 
					end.getTime().toString(), false);
			
			start.set(2012, 1, 10, 12, 0);
			start.set(2012, 1, 12, 20, 0);
			calJackWork.addEvent("Event3", calJackWork, 
					start.getTime().toString(), 
					end.getTime().toString(), true);
