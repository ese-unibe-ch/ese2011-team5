...
public class ESECalendar extends Model {
	...
	/**
	 * Constructor for new calendars of an user. This constructor is not
	 * intended for direct use. Instead use </code>ESEFactory</code> for this.
	 * 
	 * @param calendarName
	 *            Name of the calendar to differentiate between, it does not
	 *            have to be unique
	 * @param owner
	 *            The user to which the calendar is added
	 * @see ESEEvent
	 * @see ESEUser
	...
	/**
	 * Creates a new event and checks, if the event overlaps with existing
	 * events, a warning will be given and the event will not be added.
	 * 
	 * @param eventName
	 *            Description of the event shown in the calendar
	 * @param startDate
	 *            Start date of the event as "{@code dd.MM.yyyy HH:mm}"
	 *            formatted
	 * @param endDate
	 *            End date of the event as "{@code dd.MM.yyyy HH:mm}" formatted
	 * @param isPublic
	 *            Visibility to other users as {@link String}, parsed to a
	 *            {@link Boolean}
	 */
	...
	private boolean checkEventOverlaps(ESEEvent existingEvent, ESEEvent newEvent) {
		...
		boolean endDateOverlaps = endDateLiesInBetweenExistingEvent(
				existingEvent, newEvent);
		boolean embracesEvent = eventIsSubsetOfExistingEvent(existingEvent,
				newEvent);
		boolean isInEvent = eventContainsExistingEvent(existingEvent, newEvent);

		return startDateOverlaps || endDateOverlaps || embracesEvent
				|| isInEvent;
	}
	...
	private boolean eventContainsExistingEvent(ESEEvent existingEvent,
		...
		long existingEndTime = existingEvent.getEndDate().getTime();
		long newEndTime = newEvent.getEndDate().getTime();
		return newStartTime <= existingStartTime
	 ...
	 * 
	 * @param newName
	 *            New description of the calendar
	 */
	 ...
	 * 
	 * @param eventId
	 *            The Id the event was assigned when added to the database
	 */
	 ...
	 * 
	 * @param id
	 *            The Id the calendar was assigned when added to the database
	...
	/**
	 * Provides events that last at the given day regardless of the time they
	 * start or end.
	 * 
	 * @param calendarDay
	 *            The day of interest given as a {@link String} formatted as "
	 *            {@code dd.MM.yyyy HH:mm}" while time is irrelevant
	 * @param onlyPublic
	 *            Limitation whether only events which are marked as publicly
	 *            visible should be taken into consideration
	 * @return {@link ArrayList} with the events found running at this day
	 */
	public ArrayList<ESEEvent> getListOfEventsRunningAtDay(
			@Required String calendarDay, boolean onlyPublic) {
		...
		ESEEvent pseudoEvent = new ESEEvent("CompareHelperEvent",
				calendarDay.substring(0, 10) + " 00:00", calendarDay.substring(
						0, 10) + " 23:59", this, "true");
		for (ESEEvent e : this.eventList) {
			if (checkEventOverlaps(e, pseudoEvent)
					&& (!onlyPublic || e.isPublic())) {
	 ...
	 * 
	 * @param calendarDay
	 *            The day of interest given as a {@link Date} while time is
	 *            irrelevant
	 * @param onlyPublic
	 *            Limitation whether only events which are marked as publicly
	 *            visible should be taken into consideration
	 ...
	 */
	public ArrayList<ESEEvent> getListOfEventsRunningAtDay(
			@Required Date calendarDay, boolean onlyPublic) {
		String calendarDayString = ESEConversionHelper
				.convertDateToString(calendarDay);
	...
	public Iterator<ESEEvent> getIteratorOfEventsRunningAtDay(
			@Required String calendarDay, boolean onlyPublic) {
		return this.getListOfEventsRunningAtDay(calendarDay, onlyPublic)
				.iterator();
	}
