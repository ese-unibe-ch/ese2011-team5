...
public class ESEEvent implements Comparable<ESEEvent>, Visitable {
	...
	/**
	 * List of all {@link ESECalendar} which contain this ESEEvent. The
	 * ESECalendar in which this ESEEvent was originally created is also part of
	 * this List.
	 */
	private ArrayList<ESECalendar> correspondingCalendars;
	/**
	 * List of all {@link ESEUser} that participate in this ESEEvent. The
	 ...
	 * ESEEvent.<br>
	 * If {@link #isPublic()} is <code>false</code>, only ESEUser that are
	 * <b>Contancs</b> of the {@link ESECalendar#getOwner()} of the ESEEvent can
	 * view it.
	 */
	 ...
	 * The {@link ESECalendar} creating this ESEEvent is automatically added to
	 * the List of corresponding Calendars of this ESEEvent.<br>
	 * The {@link ESEUser} that owns the ESECalendar creating this ESEEvent is
	 * automatically added to the List of participants of this ESEEvent.
	 * 
	 * @param eventName
	 *            Name of Event. Must not be empty.
	 * @param correspondingCalendar
	 *            Originated Calendar.
	 * @param startDate
	 *            Time and Date when the Event starts.
	 * @param endDate
	 *            Time and Date when the Event ends.
	 * @param isPublic
	 *            Privacy of an Event.
	 * 
	 * @throws ESEException
	 *             if correspondingCalendar is <code>null</code> or if eventName
	 *             is empty.
	 * 
	 * @see ESECalendar
	 * @see ESEUser
	...
	public boolean isEventDay(int day, int month, int year) {
		...
		if (dateAsCal.get(dateAsCal.DAY_OF_YEAR) == startCal
				.get(startCal.DAY_OF_YEAR)
				&& dateAsCal.get(dateAsCal.YEAR) == startCal.get(startCal.YEAR))
			return true;
		else if (dateAsCal.get(dateAsCal.DAY_OF_YEAR) == endCal
				.get(endCal.DAY_OF_YEAR)
	...
	/**
	 * Returns a ESECalendar specified by it's Id that contains this ESEEvent.
	 * 
	 * @param id
	 *            of searched Calendar.
	 ...
	 * However, it is only within the originated ESECalendar possible, to edit
	 * an ESEEvent.
	 * 
	 * @param correspondingCalendar
	 *            to which this ESEEvent should be copied.
	 * 
	...
	/**
	 * Removes an ESEEvent, that is not originated from the corresponding
	 * Calendar.
	 * 
	...
	/**
	 * Removes an ESEEvent from all ESECalendars, including its originated
	 * ESECalendar.
	 * 
	 * @throws ESEException
	 * 
